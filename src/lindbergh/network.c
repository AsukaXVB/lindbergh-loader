...
void dns_entry_init()
{
    ...
    ret = dns_hook_push("tenporouter.loc", config->dns_router);
    if (ret == -1)
        return;

    ret = dns_hook_push("bbrouter.loc", config->dns_router);
    if (ret == -1)
        return;

    ret = dns_hook_push("naominet.jp", config->dns_default);
    if (ret == -1)
        return;

    ret = dns_hook_push("sc.naominet.jp", config->dns_default);
    if (ret == -1)
        return;

    ret = dns_hook_push("ib.naominet.jp", config->dns_default);
    if (ret == -1)
        return;
}
